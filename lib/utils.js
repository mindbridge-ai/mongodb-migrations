...
const mongodb_legacy_1 = require("mongodb-legacy");
...
function validateConnSettings(config) {
    ...
    if (!replicaset) {
        ...
        replicaset.members.forEach((m) => {
            if (!(m === null || m === void 0 ? void 0 : m.host)) {
...
function connect(config, cb) {
    ...
    if (cb) {
        mongodb_legacy_1.MongoClient.connect(url, options, cb);
    }
    else {
        return mongodb_legacy_1.MongoClient.connect(url, options);
    }
}
